...
def draw_homography_points(img, x, resize=256, color=(255,0,0)):
    ...
    # 예외 처리: img가 적절한 3차원 배열인지 확인합니다.
    if img.ndim != 3 or img.shape[2] != 3:
        raise ValueError("Input image must be a 3-channel (H, W, C) image.")

    # Draw circles on the image with red color for src points
    [cv2.circle(img, tuple(idx.astype(int)), radius=5, thickness=-1, color=(255, 0, 0)) for idx in src]

    # Draw circles on the image with green color for dst_ideal points
    [cv2.circle(img, tuple(idx.astype(int)), radius=5, thickness=-1, color=(0, 255, 0)) for idx in dst_ideal]

    # Draw circles on the image with blue color for dst points
    [cv2.circle(img, tuple(idx.astype(int)), radius=5, thickness=-1, color=(0, 0, 255)) for idx in dst]
